...
function encrypt(text) {
  const cipher = crypto.createCipheriv("aes-256-import express from "express";
import jwt from "jsonwebtoken";
import crypto from "crypto";
import Razorpay from "razorpay";
import db from "./db.js";
...
// Create Razorpay order + internal order + order_items
router.post("/razorpay/create-order", auth, async (req, res) => {
  try {
    ...
    // 4️⃣ Update orders table with Razorpay info
    await new Promise((resolve, reject) => {
      db.run(
...
cbc", Buffer.from(ENC_KEY), Buffer.from(IV));
  let encrypted = cipher.update(text, "utf8", "hex");
  encrypted += cipher.final("hex");
  return encrypted;
}
